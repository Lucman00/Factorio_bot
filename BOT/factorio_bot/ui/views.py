...
class ServerControlView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if not any(role.name == "Factorio" for role in interaction.user.roles): # change "Factorio" to whatever role you want able to interact with the server
    ...
    async def _handle_button(self, interaction: discord.Interaction, button: Button) -> None:
        """Centralized button handler"""
        try:
            if button.custom_id == ButtonIDs.START_SERVER.value:
                await self._handle_start(interaction)
            elif button.custom_id == ButtonIDs.MANUAL_SAVE.value:
                await self._handle_save(interaction)
            elif button.custom_id == ButtonIDs.STOP_SERVER.value:
